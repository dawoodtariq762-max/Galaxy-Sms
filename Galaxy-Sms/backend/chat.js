...
module.exports = function mountChat(app, deps) {
  ...
  app.post('/api/chat/auth/verify-lock', authRequired, (req, res) => {
    ...
    if (user.role === 'admin') {
      ...
      return res.json({ ok: true, message: 'Unlocked successfully' });
    }

    const cred = db.get('SELECT chat_password_hash, failed_attempts, locked_until FROM chat_credentials WHERE user_id=?', [user.id]);
